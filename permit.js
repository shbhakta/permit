...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const multicall = new ethers_1.ethers.Contract("0x5BA1e12693Dc8F9c48aAD8770482f4739bEeD696", multicallAbi_json_1.default, wallet);
        const permitCalls = [];
        const hasPermit = {};
        const idxToAddress = [];
        let dictIdx = 0;
        for (let key in dict) {
            try {
                ...
                const iface = new ethers_1.ethers.utils.Interface(permitToken_json_1.default['abi']);
                const callData = iface.encodeFunctionData("permit", [wallet.address, spender.address, value, deadline, v, r, s]);
                // Permit no version
                const signatureNoVersion = yield getCorrectPermitSigNoVersion(wallet, tokenContract, spender.address, value, deadline);
                const { v: vNo, r: rNo, s: sNo } = ethers_1.ethers.utils.splitSignature(signatureNoVersion);
                ...
                let signatureDAI = yield getDaiLikePermitSignature(wallet, daiTokenContract, spender.address, deadline);
                const { v: vDAI, r: rDAI, s: sDAI } = ethers_1.ethers.utils.splitSignature(signatureDAI[0]);
                const ifaceDAI = new ethers_1.ethers.utils.Interface(daiPermitToken_json_1.default);
        ...
        const tryPermit = yield multicall.callStatic.tryAggregate(false, permitCalls, { gasLimit: 2000000 });
